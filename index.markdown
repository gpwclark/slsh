...
| ``root::xar!`` | ``(xar! pair expression)``<br><br>Destructive form that replaces the car (first item) in a pair with a new expression.<br><br>If used on a proper list will replace the first item.  Can be used on nil to<br>create a pair (expression . nil). |
...
| ``core::append`` | ``(append l1 &rest others)``<br><br><br>Produces a new list (type will be same as first parameter) by appending the other lists onto the first. |
...
| ``core::nth`` | ``(nth idx obj)``<br><br><br>Produces the element at the provided index (0 based), error if index is out of bounds.<br>Because vectors support indexing and lists do not, this is a much faster<br>operation for a vector (uses [builtin](root::builtin?) [vec-nth](root::vec-nth)<br>on input of type vector). Supports negative indexing. |
...
| <a id="core::reduce-times" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``reduce-times``](#core::reduce-times-contents) | Lambda |
| ``core::reduce-times`` | ``(reduce-times value wrapping-fcn times)``<br><br>Apply wrapping-fcn to value number of times. Function is recursive. Recursive<br>binding for value is previous application of wrapping function to value. |
...
| ``core::setnth!`` | ``(setnth! idx obj l)``<br><br><br>Sets idx item in the vector or list to obj, produces nil or errors on invalid input.<br>This is destructive!  Because vectors support indexing and lists do not, this is<br>a much faster operation for a vector (uses [builtin](root::builtin?) [vec-setnth!](root::vec-setnth!)<br>on input of type vector). |
...
| <a id="core::slice" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``slice``](#core::slice-contents) | Lambda |
| ``core::slice`` | ``(slice lst start &rest end?)``<br><br>Returns a slice of a seq (0 based indexes, end is exclusive and optional). |
...
| <a id="core::->" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``->``](#core::->-contents) | Macro |
| ``core::->`` | ``(-> &rest args)``<br><br>inserts result of previous expression as second argument to current expression.<br>First argument is not evaluated. |
...
| <a id="core::->>" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``->>``](#core::->>-contents) | Macro |
| ``core::->>`` | ``(->> &rest args)``<br><br>inserts result of previous expression as last argument to current expression.<br>First argument is not evaluated. |
...
| ``shell::alias`` | ``(alias name body) or (alias name docstring body).``<br><br>Create an alias, intended to be used with executables not lisp code (use defn<br>for that). |
...
| <a id="root::export" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``export``](#root::export-contents) | Function |
| ``root::export`` | ``(export symbol string) -> string``<br><br>Export a key and value to the shell environment.  Second arg will be made a string and returned. |
...
| <a id="root::str-clear!" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``str-clear!``](#root::str-clear!-contents) | Function |
| ``root::str-clear!`` | ``(str-clear! string) -> string``<br><br>Clears a string.  This is a destructive form.<br><br>Returns the string it was given. |
...
| <a id="root::str-push!" class="anchor" aria-hidden="true" href="#sl-sh-form-documentation"></a>[``str-push!``](#root::str-push!-contents) | Function |
| ``root::str-push!`` | ``(str-push! string arg0 ... argN) -> string``<br><br>Push the args (as strings) onto the string.  This is a destructive form.<br><br>Arguments will be turned into strings.  Returns the string it was given. |
...
| ``root::vec-nth`` | ``(vec-nth index vector)``<br><br>Get the nth element (0 based) of a vector. If you need the equivalent operation<br>on a list use [nth](root::nth). |
...
| ``root::vec-setnth!`` | ``(vec-setnth! index value vector)``<br><br>Set the nth index (0 based) of a vector to value. This is destructive! If you<br>need the equivalent operation on a list use [setnth!](root::setnth!). |
...
version: sl-sh 0.8.62 (master:96a9598, release build, linux [x86_64], Jun 04 2020, 04:39:32 UTC)
